...
func stringtoFloatBandwidth(bandwidth string) float64 {
	Bandwidth, err := strconv.ParseFloat(bandwidth, 64)
	if err == nil {
		return Bandwidth
	}
...
func getMinRtt(nodeList []k8sApi.Node) (float64, k8sApi.Node) {
	minRtt := math.MaxFloat64
	var pick k8sApi.Node
	for _, node := range nodeList {
		...
		rttFloat, _ := strconv.ParseFloat(rtt, 64)
		minRtt = math.Min(minRtt, rttFloat)
		if minRtt == rttFloat {
			pick = node
		}
	}
...
func getNodeList(nodes *k8sApi.NodeList, bandwidth float64) []k8sApi.Node {
	var nodeList []k8sApi.Node
	for _, node := range nodes.Items {
		nodeBandwidthS := node.Labels["bandwidth"]
		nodeBandwidth := stringtoFloatBandwidth(nodeBandwidthS)
		if nodeBandwidth >= bandwidth{
...
func updateBandwidthLabel(label string, kubeClient kubernetes.Interface, nodes listersv1.NodeLister, candidateNode *k8sApi.Node) error {
	...
	prevLabel := nodeLabels["bandwidth"]
	nodeLabels["bandwidth"] = label
